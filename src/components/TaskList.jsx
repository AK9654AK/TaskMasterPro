                ...
                .map(todo => {
                        return ( < div key = { todo.id }
                            ...
                            Your task has priority: {
                                todo.priority === 'low' && < span className = "tasklist__priority" > < FontAwesomeIcon icon = { faStar }
                                /> </span >
                            } {
                                todo.priority === 'medium' && < span className = "tasklist__priority" > < FontAwesomeIcon icon = { faStar }
                                /><FontAwesomeIcon icon={faStar} / > < /span>
                            } {
                                todo.priority === 'high' && < span className = "tasklist__priority" > < FontAwesomeIcon icon = { faStar }
                            ...
                            onClick = {
                                () => handleStartDelete(todo.id) }
                            /> <
                            /div> {
                                deleteTaskId === todo.id && deleteTask ? < DeleteTask handleCancelDelete = { handleCancelDelete }
                                todoId = { todo.id }
                                dispatch = { dispatch }
                                darkMode = { darkMode }
                                />: null} {
                                    editTaskId === todo.id && edit ? < EditForm dispatch = { dispatch }
                                    todoId = { todo.id }
                                    oldValue = { todo.value }
                            ...
                            }): < p > You currently have no active tasks, click < strong style = {
                            { textDecoration: 'underline' } } > 'ADD TASK' < /strong> to add a task</p >
                    }
